...
<div align="center">
    <table id="detailedReportTable" style="border-collapse:collapse; max-width:900px; text-align: center;">
        ...
        @foreach($leads as $lead)
            @if($lead->agent_id == 0)
                <tr>
                    <td><span id="agent-name">{{$lead->agent_name}}</span></td>
                    <td><span id="time-sent">{{\Carbon\Carbon::parse($lead->last_lead)->format('m/d/Y g:i A')}}</span></td>
                    <td><span id="leads-sent">{{$lead->leads_count}}</span></td>
                    <td><span id="leads-sent">{{$lead->leads_reassigned}}</span></td>
                    <td><span id="leads-rejected">{{$lead->leads_rejected}}</span></td>
                </tr>
            @else
                <tr>
                    ...
                    <td style="@if($i % 2 != 0) {{'background-color:#EEE;'}} @else {{''}} @endif height:28px;border:solid darkgray 1px"><span id="leads-sent">{{$lead->leads_reassigned}}</span></td>
            ...
            @php
				...
				$reassignedTotal += $lead->leads_reassigned;;
            @endphp
        @endforeach
        </tbody>
        <tfoot>
        <tr>
            ...
            <th colspan="1" style="text-align:center"><b>{{$leadsTotal}}</b></th>
            <th colspan="1" style="text-align:center"><b>{{$reassignedTotal}}</b></th>
            <th colspan="1" style="text-align:center"><b>{{$rejectedTotal}}</b></th>
        </tr>
        </tfoot>
    </table>
</div>
