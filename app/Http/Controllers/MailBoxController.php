...
class MailBoxController extends Controller {
			...
			foreach ( $aMessage as $oMessage ) {
				...
				if ( $isExistingLead ) {
					...
					if ( $isMessageSpam ) {
						...
						Mail::to( $this->lead->agent()->first()->email )->bcc( 'timbrownlaw@gmail.com' )->send( new ErrorMail( $this->lead, 'Failed Reassignment Attempt: ' . explode( '!', $emailBodyFirstWord )[0] . '. Please check the spelling.' ) );
					}
	 ...
	 *       Once we hit it, we can simply leave...
	 */
...
//                        break 2;
	...
	public function sendLeads() {
		$user = Auth::user();
		if ( ! $user->is_admin ) {
			$agent_lead_count = LeadMails::where( 'agent_id', $user->id )->where( 'updated_at', '>', Carbon::now()->subDay() )->count();
			if ( $agent_lead_count >= $user->leads_allowed ) {
		...
		// Get the Lead
		$leadMails = $this->get_lead_queue( $user->user_group ); // Array
...
//        dd(DB::getQueryLog());
	 ...
	 */
	public
	function sendIndividualLead(
		int $type, $leadId, $user, string $forceEmail = '', $adminMessage = ''
	) {
		$mailable = '';
...
//                    COUNT(*)-SUM(CASE WHEN IFNULL(LM.old_agent_id, 0) > 0
//                    THEN 1
//                      ELSE 0
//                    END
//                ) AS leads_count,
//                    SUM(CASE WHEN IFNULL(LM.old_agent_id, 0) > 0 AND LM.rejected = 0
//                        THEN 1
//                          ELSE 0
//                        END
//                    ) AS leads_reassigned,
...
//		) );
		...
		$leads = $this->getReportLeadsFromDB($request, $dateFrom, $dateTo );
		return $leads;
	}

	private function getReportLeadsFromDB( Request $request, $dateFrom, $dateTo ) {
		$leads = \DB::select( \DB::raw( "
			SELECT 
				LM.agent_id,
				U.name as agent_name,
				SUM(rejected) as leads_rejected,
				SUM(IF(IFNULL(old_agent_id,0) > 0 AND rejected!=1,1,0)) as leads_reassigned,
				SUM(IFNULL( old_agent_id,0) = 0 AND rejected=0 AND agent_id > 0) as leads_count,
			    MAX(LM.updated_at) AS last_lead
			FROM lead_mails LM
				INNER JOIN users U ON U.id=LM.agent_id
				WHERE   
					LM.updated_at >= '" . $dateFrom . " 00:00:00' AND
					LM.updated_at <= '" . $dateTo . " 23:59:59'	
				GROUP BY LM.agent_id, U.name
			UNION ALL
			SELECT LM.agent_id,
...
//                    U.name AS agent_name,
//                    COUNT(*) AS leads_count,
//                    SUM(IF(IFNULL(LM.old_agent_id, 0) > 0, IF(LM.rejected!=0), 0))
//
//                    ) AS leads_reassigned,
//                    SUM(LM.rejected) AS leads_rejected,
//                    MAX(CONVERT_TZ(LM.updated_at, '+00:00', '-05:00')) AS last_lead
...
//		) );
	 ...
	 */
	private
	function detect_console_command() {
		// Added for debug
		if ( config( 'app.debug' ) ) {
	 ...
	 */
	private
	function connect_to_imap_server() {
